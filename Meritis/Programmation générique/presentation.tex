...
\begin{frame}[allowframebreaks]
\frametitle{Abstraction représentationelle}
\framesubtitle{\cite{musser_library_1987}}
\todo{now}
\end{frame}

\begin{frame}[allowframebreaks]
\frametitle{Abstraction en Java}
\todo{notes à revoir:}
\begin{itemize}
 \item Classe impose implémentation
 \item Classe abstraite impose impl. partielle
 \item Interface impose structure
 \item Ensemble de méthodes génériques plus abstrait
 \begin{itemize}
  \item classe utilitaire $\rightarrow$ classe d'opérations $\rightarrow$ ensemble d'opérateurs $\rightarrow$ interfaces fonctionnelles $\rightarrow$ lambdas
 \end{itemize}
 \item Incapable d'implémenter interface $\Rightarrow$ opérateur = 1 instance, wrapper = 1 par instance à wrapper
 \item Capable d'implémenter interface $\Rightarrow$ \todo{interface classique plus intéressante ?}
\end{itemize}
\end{frame}
